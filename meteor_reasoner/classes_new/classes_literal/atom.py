...
from meteor_reasoner.classes_new.term import Term

class Atom(AbstractLiteral):
    """
    An Atom class object consists of a predicate, a tuple of terms (constants or variables) and optionally a temporal interval,
    e.g., A(X, mike)@[1,2] or A(X, mike)
    """
    def __init__(self, predicate : str, entity: list =None, interval: Interval =None):
        self.predicate = predicate
        if all( isinstance(term, Term) for term in entity ) :
            self.entity = entity
        else :
            raise Exception("{} has an incorrect syntax!".format(entity))
    ...
    # MvN: I don't see the need to ever (re-)set the entity of a created Atom. Perhaps in grounding?
